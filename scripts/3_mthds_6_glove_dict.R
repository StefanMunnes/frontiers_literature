...
# function to filter unique pos. or neg. words above given cosinus similarity in same amount
...
# ---- 2. self-trained GloVe models with min & max pre-processing  ----
glove_ls <- readRDS("../data/glove_word_vectors.RDS")

load("../data/glove_seeds.Rdata")


# hand coded seeds
dict_hc_min <- seeds2dicts(list(onegram = glove_ls$onegram$min,
                                bigram  = glove_ls$bigram$min), seeds_hc_min)
dict_hc_max <- seeds2dicts(list(onegram = glove_ls$onegram$max,
                                bigram  = glove_ls$bigram$max),seeds_hc_max)
...
# rize and zorn seeds
dict_rz_min <- seeds2dicts(list(onegram = glove_ls$onegram$min,
                                bigram  = glove_ls$bigram$min), seeds_rz_min)
dict_rz_max <- seeds2dicts(list(onegram = glove_ls$onegram$max,
                                bigram  = glove_ls$bigram$max), seeds_rz_max)
...
# ---- 3. pre-trained german GloVe word vectors from deepset ----
